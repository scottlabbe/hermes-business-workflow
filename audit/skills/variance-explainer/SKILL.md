...
Use this skill when the user provides two trial balance files, or a prior data reliability stage identifies two reviewed trial balance tables, and wants a prior-year vs current-year variance analysis workbook. The skill produces a single Excel workbook combining both inputs with a ranked Comparison tab, variance notes, and client follow-up questions.
...
Create one primary output file and one supporting metadata file:
...
`variance_analysis.xlsx` is the human review aid. `variance_analysis_metadata.json` records input files, parameters, mappings, checks, limitations, and human review items.

Do not create any other durable files in the variance stage folder. Human-readable stage summaries belong in the workbook `Summary` sheet, and machine-readable run, validation, and handoff details belong in `variance_analysis_metadata.json`.

## Input Contract

Preferred input is the prior stage metadata JSON:

```text
runs/<run_id>/01_data_reliability/data_reliability_metadata.json
```

Read `downstream_handoffs.variance_analysis` and use its `prior_year` and `current_year` file/sheet pointers. If the user requests a variance-only workflow without a data reliability stage, read the two source files directly from `00_source/` and document that no prior data reliability stage was used.

If prior-year and current-year roles cannot be inferred safely from metadata, request text, or file names, stop and write blocked metadata that asks the user to identify each file's role.
...
Document the balance-convention assumption in the `Summary` sheet.
...
Create `variance_analysis_metadata.json` with this shape:

```json
{
  "run_id": "2026-06-01_trial_balance",
  "stage_id": "02_variance_analysis",
  ...
  "source_inputs": {
    ...
    "handoff_key": "variance_analysis"
  },
  "artifacts_written": [
    "02_variance_analysis/variance_analysis.xlsx",
    "02_variance_analysis/variance_analysis_metadata.json"
  ],
  "parameters": {
  ...
  "downstream_handoffs": {
    "variance_review": {
      "file": "02_variance_analysis/variance_analysis.xlsx",
      "sheet": "Comparison",
      "table": "Comparison!A1:O1",
      "record_count": 0
    }
  },
  "workflow_stages": [
    ...
    },
    {
      ...
      "skill": "variance-explainer",
      "status": "completed_with_limitations",
      "output": "02_variance_analysis/variance_analysis.xlsx",
      "metadata": "02_variance_analysis/variance_analysis_metadata.json"
    }
  ]
}
```
...
10. Summary documents thresholds, mappings, aggregation counts, and balance convention.
11. `variance_analysis_metadata.json` records artifacts written, source inputs, parameters, validation checks, limitations, human review items, and downstream handoffs.
