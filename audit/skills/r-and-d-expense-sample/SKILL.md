# R&D Expense Sample Skill

## Purpose

This skill creates a documented random transaction sample review aid from the source table identified by the data reliability stage.

For this demo, the R&D expense sample is an unfiltered random sample of 60 records from the full input population. Do not filter by vendor, amount, funding type, department, category, description, program, or any R&D-specific field unless the user explicitly overrides this skill.
...
Read the prior stage metadata JSON, normally `runs/<run_id>/01_data_reliability/data_reliability_metadata.json`, and use `downstream_handoffs.r_and_d_expense_sample` when present. If that key is not present, use `downstream_handoffs.expense_sample`. If the request explicitly identifies another metadata file or input workbook, use that override and document it.
...
The input table must contain at least one row and one column. No vendor, amount, funding, category, program, or R&D classification field is required for the default sample.
...
Do not apply the original top-vendor filter. Do not create a filtered population. Do not exclude records from the sampling population unless the user explicitly provides an exclusion rule.

If the input population contains fewer than 60 records, select all records and document the limitation in the metadata file.
...
```text
runs/<run_id>/02_r-and-d-expense-sample/
```
...
```json
{
  ...
  "stage_id": "02_r-and-d-expense-sample",
  "skill": "r-and-d-expense-sample",
  ...
  "source_input": {
    ...
    "handoff_key": "r_and_d_expense_sample"
  },
  "artifacts_written": [
  ...
  "downstream_handoffs": {
    "sample_review": {
      "file": "02_r-and-d-expense-sample/sample_selection.xlsx",
  ...
  "workflow_stages": [
    ...
    },
    {
      ...
      "skill": "r-and-d-expense-sample",
      "status": "completed_with_limitations",
      "output": "02_r-and-d-expense-sample/sample_selection.xlsx",
      "metadata": "02_r-and-d-expense-sample/sample_metadata.json"
    }
  ]
}
```
...
- `downstream_handoffs.r_and_d_expense_sample` or `downstream_handoffs.expense_sample` is present, unless the user explicitly supplied another input
- `ready_for_sampling` is `true`
- input file exists
- input sheet exists
- input row count is greater than zero
- no population filters were applied
- selected records come from the full input population
- selected sample count equals 60 unless the input population is smaller
...
- Do not identify records excluded by filters, because the default population is unfiltered.
